...
def ComputeSSD(TODOPatch, TODOMask, textureIm, patchL):
	...
	for r in range(ssd_rows):
		...
		for c in range(ssd_cols):
			...
			# iterate through the patch array given
			for rows in range(patch_rows):
			   for columns in range(patch_cols):	
			   	   # third dimension is ignored if first 2 dimensions match		       
			       if TODOMask[rows][columns] == 0:	
			       	 # calculate position
			         pos_r = r + rows			         
			         pos_c = c + columns
			         
			         # get the data if position is within the domain and range of
			         # the function
			         if(pos_r < patch_rows and pos_c < patch_cols):
        			         dataPatch = TODOPatch[pos_r][pos_c] 
        			         dataTexture = textureIm[pos_r][pos_c]
        			                			         
     			             			# square differential calculation 
                                         for p in range(min(len(dataPatch), 3)):
                                            d = dataPatch[p] - dataTexture[p]
                                            ssdVal = d ** 2 
                                            SSD[r][c] = ssdVal			         
			pass
		pass
...
im = Image.open('/Users/aurlin/Desktop/425/HW3/assign/boat.jpg').convert('RGB')
...
if os.path.isfile('/Users/aurlin/Desktop/425/HW3/assign/fill_region.pkl') and os.path.isfile('/Users/aurlin/Desktop/425/HW3/assign/texture_region.pkl'):
	fill_region_file = open('/Users/aurlin/Desktop/425/HW3/assign/fill_region.pkl', 'rb')
	fillRegion = pickle.load( fill_region_file )
	fill_region_file.close()

	texture_region_file = open('/Users/aurlin/Desktop/425/HW3/assign/texture_region.pkl', 'rb')
